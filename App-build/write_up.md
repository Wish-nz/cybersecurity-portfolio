...
A single-file, installable (PWA) web app built to train for the New Zealand Cyber Security Challenge (NZCSC) 2026. It bundles practice challenges, decoder tools, cheat sheets, and a scoring system into one self-contained `index.html`, deployable to the web and installable on Android.
Live site: `https://wish-ctf-training.netlify.app`
Source repo: `https://github.com/Wish-nz/CTF_Training-_App`
---
1. What We Built
The platform is a progressive web app (PWA) — a single HTML file that works in any browser and can be installed on a phone like a native app. It contains:
Feature	Details
17 practice challenges	Across 4 categories: Crypto, Web, Steganography, Forensics
Decoder tools	Base64, Hex, ROT13, Morse, Base32, URL, Binary, ASCII (decode + encode)
...
Each challenge has a description, hint, full solution, and a flag in the format `FLAG[...]`.
Scoring
...
HTML — the page structure (7 sections: Dashboard, Cheat Sheets, Challenges, Decoder Tools, Race-Day Playbook, Past Write-ups, Tool Setup)
...
`icons/icon-192.png`, `icon-512.png`	App icons for install
`.gitignore`	Git ignore rules
Deployment Workflow: Git → Netlify
The key workflow we established was continuous deployment:
...
Problem: The project lives on a network share (`\\OPENMEDIAVAULT\...`). Git on this share initially refused to work, and HTTPS authentication via Git Credential Manager hung at a GUI prompt — making non-interactive pushes impossible.
Solution:
...
```
  ...
  ```
Lesson: On Windows/network shares, SSH deploy keys are far more reliable than HTTPS credential managers for automated/scripted pushes.
...
Gotcha we learned: The CSS media-query block must sit at the END of the stylesheet so it overrides the base rules. If it's placed earlier, the base styles win and the mobile layout breaks.
...
Root cause: This was a JavaScript syntax error. Challenge 17's `content` field contained a literal newline inside a double-quoted string:
```js
...
Problem: While testing, we found the Morse decoder always returned `????` instead of decoding.
Root cause: The `toolMorseDecode` function built its lookup map backwards:
```js
// BUG: creates a letter→morse map, but the decoder looks up morse→letter
for (let k in MORSE) inv[MORSE[k]] = k;
```
The fix: Invert the map so it goes morse→letter:
```js
for (let k in MORSE) inv[k] = MORSE[k];
```
Lesson: This bug was pre-existing (not caused by the challenge 17 update), but it only surfaced because we were doing thorough end-to-end testing. Testing every tool with real inputs catches bugs that "look fine" in the code.
...
```
Once the phone cached the broken version, it kept serving that broken copy forever — it never checked the server for updates. This is a classic PWA pitfall.
...
One-time user action: Because the phone already had the broken version cached, the user needed to clear the site data (or uninstall/reinstall the PWA) once. After that, the network-first strategy means future updates flow through automatically.
...
---
Built for the NZCSC 2026 training program. Live at wish-ctf-training.netlify.app.
