...
## Portfolio activity: NIST CSF applied to an ICMP flood DoS Atack
...
- Attack type: ICMP flood or DoS
- Affected assets: core switches or routers, internal application servers, DNS, authentication systems
...
- Centralize logs. firewalls, switches, servers and IDS into SIEM with meaningful alerts for volumetric anomalies
...
- Immediate containment: implement perimeter rate-limiting or block offending protocols or sources, isolate affected segments
- Prioritise restoring services by business criticality. bring key infrastructure online first
- Collect forensic evidence: firewall rules, flow logs, packet captures, IDS alerts, and a timeline of changes and actions
- Communication: notify stakeholders. IT leadership, affected business units, customers if required, with clear status updates
- Short-term eradication: remove insecure and temporary rules, validate source filtering, block or sinkhole confirmed malicious IPs
...
- Conduct a formal post-incident review and update NIST CSF documentation. Identify, Protect, Detect, Respond and Recover with measurable follow-ups
...
**Skills demonstrated**: Incident Response, Network Analysis, NIST CSF, Firewall Management, IDS and IPS tuning, SIEM and logging, Incident Communication
